...
- run-epic pause point now comes after commit + promote, not before. Next phase is always loaded in plan.md before asking "continue or clear"
...
Completed plans are now automatically overwritten without prompting (Fixes #14).

#### Changes
- `/devloop:from-issue`: No longer prompts when existing plan is 100% complete
- `/devloop:spike`: No longer prompts about archiving completed plans
- Incomplete plans still trigger confirmation prompts (preserves data safety)

This reduces friction when starting new work after completing a plan.
