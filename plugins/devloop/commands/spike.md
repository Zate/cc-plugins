...
**If plan is complete (`complete: true`):** Proceed silently - no prompt needed for completed plans.

**If plan is incomplete (`complete: false` with pending tasks):** The spike doesn't overwrite plans - it creates a spike report. Proceed to next steps.
